...
export default async function BackstageDashboard() {
  ...
  const stats = [
    ...
    {
      label: 'Livros',
      value: backendObras.filter((o) => o.categoria === 'livro').length,
      icon: (
        <svg className="h-5 w-5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    {
      label: 'Artigos',
      value: backendObras.filter((o) => o.categoria === 'artigo').length,
      icon: (
        <svg className="h-5 w-5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    {
      label: 'Teses',
      value: backendObras.filter((o) => o.categoria === 'tese').length,
      icon: (
        <svg className="h-5 w-5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  const recentes = [...backendObras].sort((a, b) => b.ano - a.ano).slice(0, 5);
    ...
    <div className="mx-auto max-w-5xl p-6 lg:p-8">
      ...
      <div className="overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm">
        ...
        <div className="divide-y divide-zinc-100">
          {recentes.length === 0 && (
            <div className="px-6 py-8 text-center text-sm text-zinc-400">Nenhuma obra cadastrada ainda.</div>
          )}
          {recentes.map((obra) => (
            <Link
